...
static int spill_open(pending_member_t *pm, int slot_id)
{
...

    {
        ...
        logmsg_debug("NFSIS020D", "spill_open: %s(%s) spilled to %s",
...
static int spill_sync(pending_member_t *pm)
{
...

    {
        ...
        if (fclose(pm->spill.fp) != 0) {
            logmsg_error("NFSIS030E", "Spill file fclose failed: %s", strerror(errno));
            pm->spill.fp = NULL;
            errno = EIO;
            return -1;
        }
...
static int spill_load_block(pending_member_t *pm, uint32_t b)
{
    ...
    if (fseek(pm->spill.fp, (long)b * SPILL_BLK, SEEK_SET) != 0) {
        logmsg_error("NFSIS050E", "Spill file fseek(load) block %u failed: %s", b, strerror(errno));
        errno = EIO;
        return -1;
    }
...
static int spill_store_block(pending_member_t *pm, uint32_t b)
{
    ...
    if (fseek(pm->spill.fp, (long)b * SPILL_BLK, SEEK_SET) != 0) {
        logmsg_error("NFSIS060E", "Spill file fseek(store) block %u failed: %s", b, strerror(errno));
        errno = EIO;
        return -1;
    }
    w = fwrite(g_spill_block, 1, SPILL_BLK, pm->spill.fp);
    if (w != SPILL_BLK) {
        logmsg_error("NFSIS070E", "Spill file store block %u short write (%u of %d)",
               ...
               uint8_t *dst, uint32_t len)
{
    ...
    if (fseek(pm->spill.fp, (long)off, SEEK_SET) != 0) {
        logmsg_error("NFSIS080E", "Spill file read: fseek to %u failed: %s",
                  off, strerror(errno));
        errno = EIO;
        return -1;
    }
    ...
    if (r != (size_t)len) {
        logmsg_error("NFSIS090E", "Spill file read: short read at %u (%u of %u)",
                  off, (unsigned)r, len);
        errno = EIO;
        return -1;
    }
...
{
    if (pm->spill.fp != NULL) {
        logmsg_debug("NFSIS100D", "Closing spill file for %s(%s) ...",
