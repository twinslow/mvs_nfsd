...
int mvs_path_type(const char *path, int *export_idx) {
    int i;
    int exports_count = exports_count();        /* export index number is zero based. */
    export_t *nfs_export;
    ...
    for (i = 0; i < exports_count; i++) {
        ...
        size_t export_path_len = strlen(nfs_export->export_path);
...
int mvs_get_pds_dsn_and_member(
    ...
    int                 export_idx) 
{
    const char *export_path = export_get(export_idx)->export_path;
    const char *host_path = export_get(export_idx)->host_path;
